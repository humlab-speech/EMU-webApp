...
export class HistoryService {
	...
	initDeps(deps: { SsffDataService: any; LevelService: any; LinkService: any; ConfigProviderService: any; ViewStateService: any; SoundHandlerService: any; LoadedMetaDataService: any; PublisherService: any }) {
	...
	*/
	...
	public undo() {
		if (this.undoStack.length > 0) {
			// add to redo stack
			var oldChangeObj = deepCopy(this.undoStack[this.undoStack.length - 1]);
	...
	public redo() {
		if (this.redoStack.length > 0) {
			var oldChangeObj = deepCopy(this.redoStack[this.redoStack.length - 1]);
