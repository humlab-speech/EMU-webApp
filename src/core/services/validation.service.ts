	...
	public semCheckLoadedConfigs(EMUwebAppConfig, DBconfig) {
		...
		EMUwebAppConfig.perspectives.forEach((p) => {
			if (keepGoing) {
				...
				var tDefRes = this.checkIfSsffTracksAreDefined(deepCopy(p.signalCanvases.order), DBconfig);
				if (tDefRes !== true) {
				...
				var lDefRes = this.checkIfLevelsAreDefined(deepCopy(p.levelCanvases.order), DBconfig);
				if (lDefRes !== true) {
